 ...
 * @author cradeloso
 */
public class Stats_AmsterdamShroud implements PlugInFilter{
	...
	protected ImageProcessor ip2;
        protected ImageProcessor ip_aux;
        ...
        public int setup(String argv, ImagePlus imp2){
		this.imp2 = imp2;
	    ...
	    APini = 40;
            ...
            dUmbral = 0.999;
            iInhales = 10;
            sTitle = imp2.getTitle();
            imp2 = NormAmsShroud(imp2);
            imp2.show();
        ...
        public ImagePlus NormAmsShroud(ImagePlus imp){
		...
		int dimension = width*height;
                ...
                float ipslice[] = new float[dimension];
                ipslice = (float[])ip2.getPixels();
                float Vmax[] = new float[width];
                ...
                for(int i=1;i<=depth;i++) //recorro los slices
		{
			...
			for(int j=0;j<height;j++) //recorro por filas
			{
                ...
                */
                ...
                for(int i=0;i<width;i++)//recorro la anchura --> columnas
                {
                    for(int j=0;j<height;j++) //recorro la altura --recojo la columna
                    {
                        ColumnTmp[j]=ipslice[i+width*j];
                    }
                    Vmax[i] = getMaximoFloat(ColumnTmp);
                }
                //reescribo el Ams_Schroud normalizado
                for(int i=0;i<width;i++)//recorro los slices
                {
                    for(int j=0;j<height;j++) //recorro las filas
                    {
                        ipslice[i+j*width]=(float)(ipslice[i+j*width]/Vmax[i]);
                        //Ams_Schroud[depth*j+(i-1)]=Ams_Schroud[depth*j+(i-1)]/Vmax[i-1];
                    }
                }
                ip_aux = new FloatProcessor(width,height, ipslice,null);
                imp2 = new ImagePlus("Amsterdam Shroud",ip2);
                return imp2;
        }
        ...
        public float[] getInhalesX(float[] X, float[] Y, float[] InhalesY){
            ...
            for(int k=0;k<InhalesY.length;k++){
                bucle1: 
                for(int i=i_offset;i<Y.length;i++){
                    if(Y[i]==InhalesY[k]){
                        InhalesX[k]=X[i];
                        i_offset = i+iSmooth;
                        break bucle1;
